...
class Client {
	 ...
	 * @var array
	 */
	private $methods = array( 'get', 'post', 'patch', 'put', 'delete' );
	...
	private function createSavedRequest( array $requestData, $retryOnLimit = false ) {
		return array_merge( $requestData, array( 'retryOnLimit' => $retryOnLimit ) );
	}
	...
	private function createCurlMultiHandle( array $requests ) {
		...
		return array( $channels, $multiHandle );
	}
	 ...
	 * @throws InvalidRequest
	 */
	public function makeAllRequests( array $requests = array() ) {
		if ( empty( $requests ) ) {
